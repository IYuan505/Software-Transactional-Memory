...
static inline segment_t* find_target_segment(region_t* region, transaction_t* trans,
    const void* pos)
{
    segment_entry_t* segment_entry = region->segment_entry;
    while (segment_entry != NULL) {
        if (pos >= segment_entry->segment->start && pos < (char*)segment_entry->segment->start + segment_entry->segment->size) {
...
static inline write_entry_t* has_written(transaction_t* trans, atomic<int>* lock)
{
...
static inline bool commit(region_t* region, transaction_t* trans)
{
    ...
    if (trans->is_ro == false) {
        ...
        while (write_entry != NULL) {
            memcpy(write_entry->position, write_entry->new_data, write_entry->size);
            free(write_entry->new_data);
...
bool tm_read(shared_t shared, tx_t tx, void const* source, size_t size, void* target) noexcept
{
    ...
    if (write_entry != NULL) {
        int offset = (char*)source - (char*)target_segment->start;
        memcpy(target, ((char*)write_entry->new_data) + offset, size);
        return true;
    }
    ...
    if (in_read == false && write_entry==NULL) {
...
bool tm_write(shared_t shared, tx_t tx, void const* source, size_t size, void* target) noexcept
{
    ...
    restart:
        ...
        if (lock_value & WRITE_BIT || cur_timestamp > trans->start_timestamp) {
            rollback(trans);
            return false;
        }
        ...
        write_entry->new_data = malloc(target_segment->size);
        if (unlikely(write_entry->new_data == NULL)) {
            rollback(trans);
            return false;
        }
        memcpy(write_entry->new_data, target_segment->start, target_segment->size);
        write_entry->position = target_segment->start;
        write_entry->size = target_segment->size;
        write_entry->next = trans->write_entry;
        trans->write_entry = write_entry;
    }

    int offset = (char*)target - (char*)target_segment->start;
    memcpy(((char*)write_entry->new_data) + offset, source, size);
    return true;
}
