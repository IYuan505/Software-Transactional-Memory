...
/** Define the default read write entry capacity
 *  and global read write set pool
 **/
#define DEFAULT_READ_CAPACITY 8
#define DEFAULT_WRITE_CAPACITY 4
#define GLOBAL_READ_WRITE_SET_POOL 32
#define GLOBAL_READ_WRITE_SET_POOL_MASK 31

/** Define the name of the used struct **/
typedef struct segment_entry segment_entry_t;
typedef struct read_entry read_entry_t;
typedef struct read_set read_set_t;
typedef struct write_entry write_entry_t;
typedef struct write_set write_set_t;
...
struct read_set {
    ...
    atomic<int> read_set_lock;
    short read_cnt; /* number of read entries */
    short read_capacity; /* size of the read_entry array */
};

struct write_set {
    write_entry_t* write_entry; /* write set of the transaction, a dynamic array */
    atomic<int> write_set_lock;
...
struct transaction { /* Transaction */
    ...
    segment_t* last_accessed_segment; /* last accessed segment, improve locality. */
...
struct region { /* Region, central control unit */
    segment_entry_t* segment_entry; /* allocated segment of the region, a linked list. */
    segment_entry_t* freed_segment_entry; /* freed segment of the region, a linked list (Delay
    ...
    int global_read_set_counter; /* counter for the next search position */
    read_set_t
        global_read_set_pool[GLOBAL_READ_WRITE_SET_POOL]; /* global read set pool, reuse for
                                                            every transaction */
    int global_write_set_counter; /* counter for the next search position */
    write_set_t global_write_set_pool[GLOBAL_READ_WRITE_SET_POOL]; /* global write set pool */
...
static inline segment_entry_t* alloc_segment(size_t size, size_t align)
{
    ...
    /* The size of the allocation is VERSION_NUM larger, one for current valid
     ...
     in TL2). */
    if (unlikely(posix_memalign(&(segment->start), align, size * VERSION_NUM) != 0)) {
        free(segment);
        return NULL;
    }
...
find_target_segment(region_t* region, transaction_t* trans, const void* pos)
{
    ...
    if (likely(trans->last_accessed_segment != NULL && pos >= trans->last_accessed_segment->start && pos < trans->last_accessed_segment->end)) {
    ...
    while (segment_entry != NULL) {
        if (pos >= segment_entry->segment->start && pos < segment_entry->segment->end) {
...
{
    read_entry_t* read_entry = trans->read_set->read_entry;
    int read_cnt = trans->read_set->read_cnt;
    while (read_cnt--) {
...
static inline write_entry_t* has_written(transaction_t* trans,
    atomic<int>* lock)
{
    write_entry_t* write_entry = trans->write_set->write_entry;
    int write_cnt = trans->write_set->write_cnt;
    while (write_cnt--) {
...
static inline void rollback(transaction_t* trans)
{
    if (trans->is_ro) {
        ...
        /* Clean the read set. */
        atomic_store(&trans->read_set->read_set_lock, 0);
        /* Clean the write set. */
        write_entry_t* write_entry = trans->write_set->write_entry;
        int write_cnt = trans->write_set->write_cnt;
        while (write_cnt--) {
        ...
        atomic_store(&trans->write_set->write_set_lock, 0);
...
static inline bool commit(region_t* region, transaction_t* trans)
{
    if (trans->is_ro) {
        ...
        /* Fast path check, if it is the same as the start timestamp, no need to check again. */
        if (atomic_load(&region->timestamp) != trans->start_timestamp) {
            ...
            for (int i = 0; i < read_cnt; ++i) {
                ...
                if ((has_written(trans, read_entry[i].lock) == NULL && lock_value & WRITE_BIT) || cur_timestamp != read_entry[i].read_timestamp) {
                    rollback(trans);
                    return false;
                }
            }
        ...
        while (write_cnt--) {
            ...
            memcpy((char*)write_entry[write_cnt].segment->start + (1 - valid_block) * write_entry[write_cnt].segment->size + write_entry[write_cnt].block_index * BLOCK_SIZE,
                (char*)write_entry[write_cnt].segment->start + WRITE_BLOCK_INDEX * write_entry[write_cnt].segment->size + write_entry[write_cnt].block_index * BLOCK_SIZE,
                write_entry[write_cnt]
            ...
            while (to_free_entry != NULL) {
                ...
                while (segment_entry != NULL) {
                    if (segment_entry->segment == to_free_entry->segment) {
                        ...
                        /* Free the segment logically. Delay the physical free until the "tm_destroy" is called. */
...
shared_t tm_create(size_t size, size_t align) noexcept
{
    ...
    /* Allocate the global read write set pool. */
    for (int i = 0; i < GLOBAL_READ_WRITE_SET_POOL; ++i) {
        region->global_read_set_pool[i].read_entry = (read_entry_t*)malloc(sizeof(read_entry_t) * DEFAULT_READ_CAPACITY);
        if (unlikely(region->global_read_set_pool[i].read_entry == NULL)){
            for (int j = 0; j < i; ++i){
                free(region->global_read_set_pool[j].read_entry);
                free(region->global_write_set_pool[j].write_entry);
            }
            free(region);
            return invalid_shared;
        }
        region->global_read_set_pool[i].read_set_lock = 0;
        region->global_read_set_pool[i].read_capacity = DEFAULT_READ_CAPACITY;
        region->global_write_set_pool[i].write_entry = (write_entry_t*)malloc(sizeof(write_entry_t) * DEFAULT_WRITE_CAPACITY);
        if (unlikely(region->global_write_set_pool[i].write_entry == NULL)){
            free(region->global_read_set_pool[i].read_entry);
            for (int j = 0; j < i; ++i){
                free(region->global_read_set_pool[j].read_entry);
                free(region->global_write_set_pool[j].write_entry);
            }
        ...
        region->global_write_set_pool[i].write_set_lock = 0;
        region->global_write_set_pool[i].write_capacity = DEFAULT_WRITE_CAPACITY;
    }
    ...
    if (unlikely(segment_entry == NULL)) {
        for (int i = 0; i < GLOBAL_READ_WRITE_SET_POOL; ++i){
            free(region->global_read_set_pool[i].read_entry);
            free(region->global_write_set_pool[i].write_entry);
        }
        free(region);
        return invalid_shared;
    }
    ...
    region->size = size;
    region->align = align;
    region->global_read_set_counter = 0;
    region->global_write_set_counter = 0;
    return region;
}
...
void tm_destroy(shared_t shared) noexcept
{
    ...
    /* Clean the gloal read write set pool. */
    for (int i = 0; i < GLOBAL_READ_WRITE_SET_POOL; ++i) {
        free(region->global_read_set_pool[i].read_entry);
        free(region->global_write_set_pool[i].write_entry);
    }
 ...
 **/
size_t tm_align(shared_t shared) noexcept { return ((region_t*)shared)->align; }
...
tx_t tm_begin(shared_t shared, bool is_ro) noexcept
{
    ...
    if (!is_ro) {
        /* Let the start search index be the global one, improve efficiency. */
        int cur_search_index = region->global_read_set_counter;
        while (true) {
            /* Search for the available read set in the global pool. */
            int expected_lock_value = 0;
            if (atomic_compare_exchange_strong(
                    &region->global_read_set_pool[cur_search_index]
                         .read_set_lock,
                    &expected_lock_value, 1)) {
                trans->read_set = &region->global_read_set_pool[cur_search_index];
                trans->read_set->read_cnt = 0;
                cur_search_index = (cur_search_index + 1) & GLOBAL_READ_WRITE_SET_POOL_MASK;
                /* Update the global search index. */
                region->global_write_set_counter = cur_search_index;
                break;
            }
            else {
                cur_search_index = (cur_search_index + 1) & GLOBAL_READ_WRITE_SET_POOL_MASK;
        ...
        cur_search_index = region->global_write_set_counter;
        while (true) {
            /* Search for the available write set in the global pool. */
            int expected_lock_value = 0;
            if (atomic_compare_exchange_strong(
                    &region->global_write_set_pool[cur_search_index]
                         .write_set_lock,
                    &expected_lock_value, 1)) {
                trans->write_set = &region->global_write_set_pool[cur_search_index];
                trans->write_set->write_cnt = 0;
                cur_search_index = (cur_search_index + 1) & GLOBAL_READ_WRITE_SET_POOL_MASK;
                region->global_write_set_counter = cur_search_index;
                break;
            }
    ...
    void* target) noexcept
{
    ...
    short block_index = ((char*)source - (char*)target_segment->start) >> BLOCK_SHIFT;
restart:
    ...
    if (trans->is_ro) {
       ...
       by other transactions. */
        if (cur_timestamp > trans->start_timestamp || lock_value & WRITE_BIT) {
         ...
         to read from. */
            if (likely(cur_timestamp > trans->start_timestamp && target_segment->archieve_timestamp[block_index] <= trans->start_timestamp) && ((lock_value & ARCHIEVE_BIT) == 0)) {
                memcpy(target,
         ...
         than the start timestamp of the transaction. */
            if (cur_timestamp > trans->start_timestamp) {
                ...
                for (int i = 0; i < read_cnt; ++i) {
                    ...
                    if ((has_written(trans, read_entry[i].lock) == NULL && read_lock_value & WRITE_BIT) || read_cur_timestamp != read_entry[i].read_timestamp) {
                        rollback(trans);
                        return false;
                    }
                }
           ...
           but for memory efficiency, we reuse it. */
            ...
            read_entry_t* read_entry = trans->read_set->read_entry + trans->read_set->read_cnt;
            read_entry->lock = lock;
            read_entry->read_timestamp = cur_timestamp;
            read_entry->block_index = block_index;
            trans->read_set->read_cnt++;
            if (trans->read_set->read_cnt == trans->read_set->read_capacity) {
                trans->read_set->read_capacity <<= 1;
                read_entry_t* new_read_entry = (read_entry_t*)realloc(
                    trans->read_set->read_entry,
                    sizeof(read_entry_t) * trans->read_set->read_capacity);
                if (unlikely(new_read_entry == NULL)) {
                    rollback(trans);
                    return false;
                }
                trans->read_set->read_entry = new_read_entry;
            }
    ...
    void* target) noexcept
{
    ...
    /* Calculate the index of the current position inside the segment. */
    short block_index = ((char*)target - (char*)target_segment->start) >> BLOCK_SHIFT;
restart:
    /* Read the lock value, get the necessary information. */
    /* "<< 1", make the lock 8 byte aligned manually */
    atomic<int>* lock = &target_segment->lock[block_index << 1];
    ...
    if (write_entry != NULL) {
       ...
       not much we can do, abort. */
        ...
        if (lock_value & WRITE_BIT || (read_entry != NULL && cur_timestamp != read_entry->read_timestamp)) {
            rollback(trans);
            return false;
        }
        ...
        write_entry = trans->write_set->write_entry + trans->write_set->write_cnt;
        write_entry->lock = lock;
        write_entry->segment = target_segment;
        write_entry->block_index = block_index;
        trans->write_set->write_cnt++;
        if (trans->write_set->write_cnt == trans->write_set->write_capacity) {
            trans->write_set->write_capacity <<= 1;
            write_entry_t* new_write_entry = (write_entry_t*)realloc(
                trans->write_set->write_entry,
                sizeof(write_entry_t) * trans->write_set->write_capacity);
            if (unlikely(new_write_entry == NULL)) {
                rollback(trans);
                return false;
            }
            trans->write_set->write_entry = new_write_entry;
        }
