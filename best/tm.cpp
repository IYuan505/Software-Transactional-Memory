...
static inline
void check_read_and_remove(transaction_t* trans, atomic<int>* lock) {
...
static inline
bool check_write(transaction_t* trans, atomic<int>* lock) {
...
bool validate_read(transaction_t* trans, atomic<int>* lock) {
    ...
    if (lock!=NULL){
        if (check_write(trans, lock)==true)
            return true;
    }
...
bool acquire_lock(transaction_t* trans, segment_t* target_seg) {
    if (check_write(trans, &target_seg->lock) == false) {
restart:
        check_read_and_remove(trans, &target_seg->lock);
...
bool tm_read(shared_t shared, tx_t tx, void const* source, size_t size, void* target) noexcept {
    ...
    if (validate_read(trans, &target_seg->lock) == false) {
        rollback(trans);
        return false;
    }
